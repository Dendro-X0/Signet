...
/// Identity, sign, explain, and release self-signed Tauri apps.
///
/// Run with no subcommand to open the interactive TUI hub.
#[derive(Debug, Parser)]
#[command(name = "selfsign", version, about, long_about = None)]
pub struct Cli {
    #[command(subcommand)]
...
pub enum Command {
    ...
    /// Build with Tauri and sign platform artifacts
    Build(build::Args),
