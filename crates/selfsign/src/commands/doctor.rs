...
fn gather_checks() -> Vec<Check> {
    ...
    let has_config = std::path::Path::new("selfsign.toml").exists();
    checks.push(Check {
        name: "selfsign.toml".into(),
        ok: has_config,
        severity: Severity::Optional,
        detail: if has_config {
            "present in cwd".into()
        } else {
            "missing in cwd — run `selfsign init`".into()
        },
    });

    let identity_ok = std::path::Path::new(".selfsign/identity/active").exists();
    checks.push(Check {
        name: "identity".into(),
        ok: identity_ok,
        severity: Severity::Optional,
        detail: if identity_ok {
            "active identity present under .selfsign/identity/".into()
        } else {
            "no active identity — run `selfsign identity create`".into()
        },
    });
    ...
    checks.push(Check {
        ...
        detail: if gh {
            ...
            "missing — install `gh` or set GH_TOKEN for `selfsign release`".into()
        },
    });
        ...
        "windows" => {
            ...
            checks.push(Check {
                ...
                detail: match signtool {
                    ...
                    None => {
                        "not found (Windows SDK Signing Tools; needed for `selfsign build`)"
                            .into()
                    }
                },
            });
...
fn print_human(checks: &[Check]) {
    ...
    console::blank();
    console::note("`selfsign release --dry-run` lists assets; publish needs gh or GH_TOKEN.");
    console::blank();
}
