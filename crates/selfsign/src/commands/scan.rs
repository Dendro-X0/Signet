...
pub struct Args {
    ...
    /// Write / update selfsign.toml from the suggested desktop signing config
    #[arg(long)]
    pub apply: bool,
...
fn apply_suggestion(report: &ScanReport, force: bool) -> anyhow::Result<()> {
    ...
    if config_path.exists() && !force {
        ...
        console::ok_line(&format!(
            "updated platforms in selfsign.toml (use --force to also replace name/tauri_root)"
        ));
        ...
        cfg.write(&config_path)?;
        console::blank();
        console::ok_line("rewrote project + platforms in selfsign.toml");
    } else {
        ...
        console::blank();
        console::ok_line("wrote selfsign.toml from scan suggestion (desktop platforms only)");
    }
    ...
    console::numbered(1, "selfsign identity create", "create a local code-signing identity");
    Ok(())
}
...
fn ensure_gitignore(root: &std::path::Path) -> anyhow::Result<()> {
    ...
    if gi.exists() {
        ...
        fs::write(gi, format!("# selfsign secrets\n{line}\n"))?;
    }
    Ok(())
}
