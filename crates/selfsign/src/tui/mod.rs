...
const ITEMS: &[HubItem] = &[
    ...
    HubItem {
        id: "init",
        label: "Init",
        hint: "Create selfsign.toml + .selfsign/",
    },
    HubItem {
        id: "identity",
        label: "Identity",
    ...
    HubItem {
        id: "build",
        label: "Build",
        hint: "Tauri build + sign (guided options)",
    },
    HubItem {
        id: "release",
        label: "Release",
...
pub fn run_hub() -> anyhow::Result<()> {
    if !io::stdout().is_terminal() {
        println!("selfsign — interactive TUI requires a terminal");
        println!("Try: selfsign --help");
        println!("Or:  selfsign doctor");
        return Ok(());
    }
...
fn draw_header(frame: &mut Frame, area: Rect) {
    let line = Line::from(vec![
        Span::styled(" selfsign ", title_style()),
        Span::styled("· self-signed Tauri releases", dim()),
    ]);
