...
pub fn guided_scan() -> anyhow::Result<()> {
    ...
    if confirm("apply suggested desktop config to selfsign.toml?", false)? {
        commands::scan::run(commands::scan::Args {
            path: PathBuf::from("."),
            json: false,
            apply: true,
...
pub fn guided_init() -> anyhow::Result<()> {
    ...
    if status.has_config {
        console::ok_line("selfsign.toml already exists");
    ...
    if !opts.skip_scan {
        if confirm("scan the repo for installers first?", true)? {
            ...
            if !ProjectStatus::probe(".").has_config
                && confirm("apply suggested selfsign.toml from scan?", true)?
            {
                commands::scan::run(commands::scan::Args {
                    path: PathBuf::from("."),
                    json: false,
