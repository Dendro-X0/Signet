//! Non-secret project configuration (`selfsign.toml`).
...
pub const CONFIG_FILE_NAME: &str = "selfsign.toml";
pub const SECRETS_DIR_NAME: &str = ".selfsign";
...
pub struct Config {
    ...
    pub release: Release,
    /// Relative path to the secrets directory (gitignored). Default: `.selfsign`
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct Release {
    /// Publish to GitHub Releases when `selfsign release` is used
    #[serde(default = "default_true")]
    pub github: bool,
...
impl Config {
    ...
    pub fn write(&self, path: impl AsRef<Path>) -> Result<(), ConfigError> {
        ...
        let header = "# selfsign project config (safe to commit)\n\
...
/// Resolve config path: explicit override, else `./selfsign.toml`.
