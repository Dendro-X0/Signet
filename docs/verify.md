...
signet verify --fail-stale              # exit 1 if sums list missing files / version mismatch
...
When `SHA256SUMS` lists artifacts that are gone (clean tree) or whose filenames carry a version different from the project version, verify **warns** with rebuild/prune guidance. Use `--fail-stale` in CI to turn that into a hard failure.

## Exit codes

| Code | Meaning |
|------|---------|
| 0 | Checks passed (stale sums only warn unless `--fail-stale`) |
| 1 | Checksum, fingerprint, signature mismatch, or `--fail-stale` |
...
- Spec: [`specs/backend/fault-tolerance-design.md`](../specs/backend/fault-tolerance-design.md)
